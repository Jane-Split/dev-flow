...
- **上下文智能管理系统（Context Manager）** - 解决"上下文限制导致敷衍生成"问题

  **核心原则：准确性优先，宁可串行也不生成错误代码**

  **最小安全上下文阈值（50KB 硬约束）**
  - 每个 develop-expert subagent 至少分配 50KB 上下文
  - 低于阈值时强制切换为串行执行
  - Step 2.5 强制读取验证必须完整执行

  **智能任务拆分（按依赖关系）**
  - 无依赖任务（Entity/Enum）：可并行执行
  - 只读依赖任务（DTO/Mapper）：通过接口契约并行执行
  - 写依赖任务（Service/Controller）：必须串行执行
  - 复杂任务（>70KB）：自动拆分为子任务

  **分层设计文档（精准裁剪）**
  - Layer 1（必须加载）：当前类定义 + 直接依赖接口签名（10-15KB）
  - Layer 2（按需加载）：间接依赖字段列表（5-10KB）
  - Layer 3（延迟加载）：完整实现细节（0-10KB）

  **执行模式动态决策**
  - 并行模式：所有任务 <=50KB 且总数 <=3
  - 串行模式：任一任务 >50KB 或上下文不足
  - 混合模式：无依赖任务并行 + 依赖链任务串行

  **动态监控与保护机制**
  - 70% 警告：提示保存进度
  - 85% 临界：强制分段执行
  - 95% 紧急：立即停止并创建 continuation task
...
- **context-manager.md** - 上下文管理器，负责任务分配、执行模式决策、动态监控

### 改进

- **orchestrator.md** - 集成 context-manager，在 DAG 调度前进行上下文评估

### 解决的问题

- ✅ 上下文限制导致 AI 用日志占位替代实际业务逻辑
- ✅ 并行任务过多导致每个任务上下文不足
- ✅ 复杂任务上下文溢出导致代码生成不完整
- ✅ 任务拆分不考虑依赖关系导致代码错误
