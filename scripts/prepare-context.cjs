...
// ============================================================
// 动态预算配置（由 calculateBudget 根据模型动态计算）
// ============================================================

let MAX_BRIEF_SIZE = 120 * 1024; // 默认值，将在 generateTaskBrief 中动态覆盖
let MAX_FILE_READ = 30 * 1024;   // 默认值，将在 generateTaskBrief 中动态覆盖
let MAX_DEPENDENCY_FILES = 15;   // 默认值，将在 generateTaskBrief 中动态覆盖
...
function collectTaskContext(taskId, demandName) {
  ...
  const budget = calculateBudget(modelName, {
    systemPromptSizeKB: estimateSystemPromptSize(taskId)
  });

  console.log(formatBudgetReport(budget));

  MAX_BRIEF_SIZE = budget.briefSizeKB * 1024;
  MAX_FILE_READ = budget.fileReadKB * 1024;
  MAX_DEPENDENCY_FILES = budget.maxDependencyFiles;
    ...
    if (contractContent) {
      ...
      if (projectRoot && deps.length > 0) {
        ...
        for (const dep of deps) {
          if (depCount >= MAX_DEPENDENCY_FILES) {
            console.warn(`[WARN] 依赖文件数已达上限 ${MAX_DEPENDENCY_FILES}，跳过剩余依赖`);
            break;
          }
          // 搜索类文件
...
function generateBrief(taskId, sections, totalSize) {
  const header = [
    ...
    `> Max Dependencies: ${MAX_DEPENDENCY_FILES} files`,
