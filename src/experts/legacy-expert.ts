...
import { writeText, fileExists, readText } from '../utils/fs-utils.js';
import * as path from 'node:path';
import { getMigrationTemplate, getRecommendedTemplates } from '../migration/migration-templates.js';
import type { MigrationTemplate } from '../migration/migration-templates.js';
...
export class LegacyExpert extends BaseExpert {
  ...
  canHandle(task: Task): boolean {
    // 0. 如果显式指定了其他专家，不匹配
    if (task.expert && task.expert !== 'LegacyExpert') return false;
    ...
    const legacyKeywords =
      /老旧|迁移|重构|legacy|migrate|refactor|jQuery|AngularJS|PHP\s*5|Java\s*[67]|Python\s*2|Backbone|Knockout|gulp|grunt|ie兼容|技术债务|tech debt/i;
   ...
   * 检测迁移类型
   */
  private detectMigrationType(task: Task): { from: string; to: string } | null {
    const desc = `${task.name} ${task.description}`.toLowerCase();

    const migrationMap: Array<{ pattern: RegExp; from: string; to: string }> = [
  ...
  private async executeMigration(
    task: Task,
    migrationType: { from: string; to: string },
    files: string[],
   ...
   * 生成迁移后的代码
   */
  private generateMigratedCode(task: Task, template: MigrationTemplate): string {
    const taskName = this.toFileName(task.name);
...
`;
   ...
   * 生成 React 代码
   */
  private generateReactCode(task: Task, taskName: string): string {
    return `/**
 * ${task.name} - React 组件
 *
 * 描述: ${task.description}
...
`;
   ...
   * 生成 Vue 代码
   */
  private generateVueCode(task: Task, taskName: string): string {
    return `<template>
  <!-- ${task.name} -->
  <!-- 描述: ${task.description} -->
  <!-- 迁移自: jQuery → Vue -->
...
`;
   ...
   * 生成 Angular 代码
   */
  private generateAngularCode(task: Task, taskName: string): string {
    return `/**
...
`;
   ...
   * 生成 Node.js 代码
   */
  private generateNodeCode(task: Task, taskName: string): string {
    return `/**
...
`;
   ...
   * 生成迁移测试
   */
  private generateMigrationTest(task: Task, template: MigrationTemplate): string {
    const taskName = this.toFileName(task.name);
    const isReact = template.to === 'React';
    const isVue = template.to === 'Vue';
    const isNode = template.to.includes('Node');
...
`;
   ...
   * 生成迁移指南
   */
  private generateMigrationGuide(task: Task, template: MigrationTemplate): string {
...
`;
   ...
   * 通用迁移代码（无匹配模板时）
   */
  private async generateGenericMigration(
    task: Task,
    migrationType: { from: string; to: string },
    files: string[],
    changes: ExpertResult['changes'],
...
`;
   ...
   * 通用老旧项目代码生成
   */
  private async generateLegacyCode(
    task: Task,
    files: string[],
    changes: ExpertResult['changes'],
    suggestions: string[]
