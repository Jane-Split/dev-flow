...
export class TestExpert extends BaseExpert {
  ...
  canHandle(task: Task): boolean {
    // 如果显式指定了其他专家，不匹配
    if (task.expert && task.expert !== 'TestExpert') return false;
    
    // 如果显式指定了TestExpert，匹配
    if (task.expert === 'TestExpert') return true;
    ...
    if (task.type !== 'test' && task.output?.files?.length > 0) {
      const nonTestFiles = task.output.files.filter(f => {
        ...
        // 如果文件名包含test，可能是测试文件
        if (ext.includes('test') || ext.includes('spec')) return true;
        // 否则可能是业务代码文件，不匹配
        return false;
      });
      // 如果没有测试相关文件，不匹配
   ...
   * 根据文件扩展名生成对应语言的测试代码
   */
  private generateTestCode(task: Task, ext: string, filePath: string): string {
   ...
   * 根据任务描述推导测试用例名称
   */
  private deriveTestCases(task: Task): string[] {
    const cases: string[] = [];
   ...
   */
  private generateVitestTest(task: Task, baseName: string, testCases: string[]): string {
   ...
   * 生成 Jest 测试代码 (.js/.jsx)
   */
  private generateJestTest(task: Task, baseName: string, testCases: string[]): string {
    const describeName = this.sanitizeDescribeName(task.name);
    const lines: string[] = [];
   ...
   */
  private generateJUnitTest(task: Task, baseName: string, testCases: string[]): string {
   ...
   * 生成 pytest 测试代码 (.py)
   */
  private generatePytestTest(task: Task, baseName: string, testCases: string[]): string {
    const lines: string[] = [];
   ...
   * 生成通用测试骨架
   */
  private generateGenericTest(task: Task, baseName: string, testCases: string[]): string {
    const lines: string[] = [];
   ...
   * 当没有指定输出文件时，生成默认测试文件
   */
  private async generateDefaultTest(task: Task, projectRoot: string): Promise<ExpertResult> {
   ...
   */
  private sanitizeDescribeName(name: string): string {
   ...
   * 清理 it 名称中的特殊字符
   */
  private sanitizeItName(name: string): string {
    return name.replace(/'/g, "\\'").replace(/\n/g, ' ').trim();
  }
