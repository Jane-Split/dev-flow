...
export interface ArchitectureResult {
  ...
  tradeoffs: { concern: string; decision: string; impact: string }[];
  documentPath: string;
}
...
export class ArchitectureAgent extends BaseAgent {
  ...
  async execute(requirement: string): Promise<AgentResult<ArchitectureResult>> {
    try {
      logger.title('架构决策');
...
${decision.options.map(opt =>
    `| ${opt.name} | ${opt.score}/100 | ${opt.recommended ? '推荐' : '备选'} | ${opt.pros.join(', ')} | ${opt.cons.join(', ')} |`
  ).join('\n')}
`;
}).join('\n')}
