...
const FRAMEWORK_OPTIONS: Record<ProjectScale, TechOption[]> = {
  small: [
    {
      name: 'Next.js',
...
const DATABASE_OPTIONS: Record<ProjectScale, TechOption[]> = {
  small: [
    {
      name: 'SQLite',
...
const CACHE_OPTIONS: Record<ProjectScale, TechOption[]> = {
  small: [
    {
      name: '无缓存 (内存变量)',
      pros: ['零成本', '实现简单', '无额外依赖'],
...
const AUTH_OPTIONS: Record<ProjectScale, TechOption[]> = {
  small: [
    {
...
const QUEUE_OPTIONS: Record<ProjectScale, TechOption[]> = {
  small: [],
  medium: [
    {
      name: 'BullMQ (Redis)',
...
const MONITORING_OPTIONS: Record<ProjectScale, TechOption[]> = {
  small: [
    {
      name: 'Console + 简单日志',
...
const DEPLOYMENT_OPTIONS: Record<ProjectScale, TechOption[]> = {
  small: [
    {
      name: 'Vercel',
...
const ARCHITECTURE_PATTERNS: Record<ProjectScale, ArchitecturePattern> = {
  small: {
    name: '单体应用',
    description:
      '所有功能集中在一个应用中，前后端可选同框架（如 Next.js），适合快速开发和迭代。部署简单，维护成本低。',
    suitableFor: [
      '个人项目和 Side Project',
      '工具类应用',
      '静态站点和内容站点',
  ...
  medium: {
    name: '前后端分离',
    description:
      '前端和后端独立开发部署，通过 RESTful API 或 GraphQL 通信。前端负责 UI 和交互，后端负责业务逻辑和数据。适合团队协作。',
    suitableFor: [
      '团队协作项目',
      'CRUD 类管理后台',
      '电商和社交平台',
  ...
  large: {
    name: '微服务架构',
    description:
      '将系统拆分为多个独立服务，每个服务负责单一业务能力，通过 API 网关和消息队列通信。适合大规模团队和高并发场景。',
    suitableFor: [
      '高并发系统',
      '多团队协作',
      '需要独立部署和扩展的业务',
...
const DIRECTORY_STRUCTURES: Record<ProjectScale, string[]> = {
  small: [
    'src/',
    '  app/                    # 页面路由 (App Router)',
    '  components/             # 组件',
...
const MODULE_BOUNDARIES: Record<ProjectScale, string[]> = {
  small: [
...
const DATA_FLOW_DESCRIPTIONS: Record<ProjectScale, string> = {
  small:
    '用户请求 -> Next.js 路由 -> 页面组件 -> 调用 lib 工具函数 -> 数据库操作 -> 返回数据 -> 组件渲染。对于 Server Component，数据获取在服务端完成；对于 Client Component，通过 API Route 获取数据。',
  medium:
    '用户请求 -> 前端路由 -> 页面组件 -> 调用 services 层 -> HTTP 请求 -> 后端路由 -> Controller -> Service (业务逻辑) -> Model (数据访问) -> 数据库 -> 原路返回。状态管理 (stores) 负责前端状态缓存和同步。',
  large:
    '用户请求 -> CDN/负载均衡 -> API 网关 -> 认证鉴权 -> 路由转发 -> 目标微服务 -> 业务处理 -> 数据库/缓存 -> 返回结果。异步操作通过消息队列解耦：服务 A 发送事件 -> 消息队列 -> 服务 B 消费处理。服务间同步调用通过 gRPC。',
};

// ─── API 契约描述预设 ───────────────────────────────────────

const API_CONTRACT_DESCRIPTIONS: Record<ProjectScale, string> = {
  small:
    '使用 Next.js Route Handlers 或 Server Actions。RESTful 风格，JSON 请求/响应。错误使用 HTTP 状态码 + 统一错误格式 { success: false, error: string }。',
  medium:
    '前后端通过 RESTful API 通信，使用 OpenAPI/Swagger 定义接口契约。请求/响应格式统一，包含分页、排序、过滤标准参数。认证使用 Bearer Token，错误使用标准错误码。',
  large:
    '外部使用 RESTful API (通过 API 网关)，内部服务间使用 gRPC/Protobuf。事件驱动使用消息队列，事件格式使用 JSON Schema 定义。API 版本化通过 URL 路径 (v1/v2)，向后兼容策略。',
};

// ─── 部署方案预设 ───────────────────────────────────────────

const DEPLOYMENT_PLANS: Record<ProjectScale, DeploymentPlan> = {
  small: {
    strategy: 'vercel',
    description:
      '使用 Vercel 进行部署，支持自动构建和预览环境。配置 vercel.json 进行路由和重写规则设置。环境变量通过 Vercel Dashboard 管理。',
    requirements: ['Vercel 账号', 'GitHub/GitLab 仓库连接', 'Node.js 18+'],
  },
  medium: {
    strategy: 'docker',
    description:
      '使用 Docker Compose 编排前后端服务和数据库。开发环境与生产环境使用同一套 Docker 配置，确保一致性。可配合 Nginx 反向代理。',
    requirements: ['Docker 和 Docker Compose', '服务器 (云服务器或本地)', '域名和 SSL 证书', 'Nginx (可选)'],
  },
  large: {
    strategy: 'k8s',
    description:
      '使用 Kubernetes 进行容器编排，配合 Helm Chart 管理部署配置。使用 Ingress Controller 处理外部流量，HPA 实现自动扩缩容。通过 CI/CD 流水线自动部署。',
    requirements: [
...
const TRADEOFFS: Record<ProjectScale, { concern: string; decision: string; impact: string }[]> = {
  small: [
    {
 ...
 * 从需求描述中评估项目规模
 */
export function getScaleFromRequirement(requirement: string): ProjectScale {
 ...
 * 根据规模获取技术选型推荐
 */
export function getTechRecommendations(scale: ProjectScale): TechDecision[] {
 ...
 * 根据规模获取架构模式
 */
export function getArchitecturePattern(scale: ProjectScale): ArchitecturePattern {
 ...
 * 根据规模和框架获取分层策略
 */
export function getLayerStrategy(scale: ProjectScale, _framework: string): LayerStrategy {
  return {
 ...
 * 根据规模获取部署方案
 */
export function getDeploymentPlan(scale: ProjectScale): DeploymentPlan {
 ...
 * 根据规模获取权衡分析
 */
export function getTradeoffs(scale: ProjectScale): { concern: string; decision: string; impact: string }[] {
