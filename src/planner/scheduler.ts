...
import type { Task, Complexity } from './task-splitter.js';
...
export interface SchedulerConfig {
  ...
  parallelEfficiency: number; // 并行效率系数 (0-1)
}
...
export class Scheduler {
   ...
   * 计算预估执行时间
   */
  private calculateEstimatedTime(levels: ExecutionLevel[]): number {
    let totalTime = 0;
  ...
  generateReport(
    ...
    const lines = [
      ...
      '',
      '执行层级:',
      ...schedule.levels.map(l =>
        `  Level ${l.level}: ${l.tasks.length} 个任务 ${l.parallel ? '(并行)' : '(串行)'}`
      ),
    ];
