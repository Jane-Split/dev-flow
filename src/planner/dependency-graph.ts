...
export class DependencyGraph {
  ...
  private adjList: Map<string, string[]>;  // 邻接表: 任务ID -> 依赖该任务的任务列表
  private inDegree: Map<string, number>;   // 入度表: 任务ID -> 入度
   ...
   * 获取任务的执行顺序（包括层级信息）
   */
  getExecutionOrder(): Array<{ task: Task; level: number }> {
    const levels = this.getExecutionLevels();
