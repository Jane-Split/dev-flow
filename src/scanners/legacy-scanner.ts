...
export interface LegacyAnalysisResult {
  isLegacy: boolean;
  legacyScore: number; // 0-100, 越高越老旧
...
const TECH_PATTERNS: Record<LegacyTechType, TechPattern> = {
  jquery: {
...
const SECURITY_PATTERNS: Array<{ pattern: RegExp; description: string; severity: TechDebt['severity'] }> = [
...
export class LegacyScanner {
   ...
   * 执行完整的老旧项目扫描
   */
  async scan(): Promise<LegacyAnalysisResult> {
   ...
   * 识别老旧技术栈
   */
  private async identifyTechStacks(): Promise<LegacyTechStack[]> {
    const results: LegacyTechStack[] = [];
    ...
    for (const [type, pattern] of Object.entries(TECH_PATTERNS) as [LegacyTechType, TechPattern][]) {
      const files: string[] = [];
      ...
      if (pattern.files) {
        for (const filePattern of pattern.files) {
          ...
          files.push(...matches);
          if (matches.length > 0) confidence += 30;
        }
      }
   ...
   * 分析代码复杂度
   */
  private async analyzeComplexity(): Promise<LegacyAnalysisResult['complexity']> {
    const sourceFiles = await this.findSourceFiles();
   ...
   * 识别技术债务
   */
  private async identifyTechDebts(techStacks: LegacyTechStack[]): Promise<TechDebt[]> {
   ...
   * 生成迁移路径
   */
  private generateMigrationPaths(techStacks: LegacyTechStack[]): MigrationPath[] {
  ...
  private async readPackageDeps(): Promise<Record<string, string>> {
  ...
  private calculateLegacyScore(techStacks: LegacyTechStack[], techDebts: TechDebt[]): number {
    let score = 0;
  ...
  private generateSummary(techStacks: LegacyTechStack[], legacyScore: number): string {
  ...
  private generateRecommendations(techStacks: LegacyTechStack[], techDebts: TechDebt[]): string[] {
    const recs: string[] = [];
  ...
  private getSecurityFixSuggestion(description: string): string {
    if (description.includes('eval')) return '使用 JSON.parse() 或其他安全替代方案';
    if (description.includes('innerHTML')) return '使用 textContent 或 DOMPurify 进行消毒';
    if (description.includes('document.write')) return '使用 DOM API 创建元素';
    if (description.includes('mysql_query')) return '使用 PDO 或 MySQLi 预处理语句';
    if (description.includes('超全局变量')) return '使用 filter_input() 或框架的请求对象';
    if (description.includes('密码硬编码')) return '使用环境变量或密钥管理服务';
    if (description.includes('HTTP')) return '升级到 HTTPS，使用相对协议或 CSP';
    return '请评估并修复此安全问题';
  }

  private estimateEffort(tech: LegacyTechStack, target: string): string {
    const base = tech.confidence >= 70 ? '3-6 个月' : '1-3 个月';
    if (target === 'React' || target === 'Vue') return base;
    if (target === 'Angular 17+') return '3-6 个月';
    if (target === 'Node.js') return '2-4 个月';
    if (target === 'Java 17+') return '1-2 个月';
    return base;
  }

  private getMigrationSteps(from: LegacyTechType, to: string): string[] {
    const commonSteps = [
      '1. 建立测试保障（目标覆盖率 > 60%）',
      '2. 搭建新技术栈基础设施',
      '3. 选择低风险模块进行试点迁移',
  ...
  private getMigrationRisks(from: LegacyTechType): string[] {
