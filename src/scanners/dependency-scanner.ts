...
export class DependencyScanner {
  ...
  private async detectProjectInfo(pkgJson: any): Promise<{ name: string; version: string }> {
    // Node.js/前端项目
    if (pkgJson.name) {
    ...
    if (await fileExists(buildGradlePath) || await fileExists(settingsGradlePath)) {
      try {
        const gradleContent = await readText(
          await fileExists(settingsGradlePath) ? settingsGradlePath : buildGradlePath
        );
  ...
  private async detectTechStack(pkgJson: any): Promise<TechStack> {
    const deps = { ...pkgJson.dependencies, ...pkgJson.devDependencies };

    // 先检测非 JS/TS 项目
    const nonJsStack = await this.detectNonJsTechStack();
    if (nonJsStack) return nonJsStack;
   ...
   */
  private async detectNonJsTechStack(): Promise<TechStack | null> {
   ...
   * 检测 Java Maven 项目
   */
  private async detectJavaMavenStack(): Promise<TechStack> {
    const stack: TechStack = { language: 'Java', framework: 'Unknown' };
    try {
   ...
   * 检测 Java Gradle 项目
   */
  private async detectJavaGradleStack(): Promise<TechStack> {
    const stack: TechStack = { language: 'Java', framework: 'Unknown' };
    try {
   ...
   * 检测 Python 项目
   */
  private async detectPythonStack(): Promise<TechStack> {
    const stack: TechStack = { language: 'Python', framework: 'Unknown' };
    const root = this.projectRoot;
   ...
   * 检测 Go 项目
   */
  private async detectGoStack(): Promise<TechStack> {
    const stack: TechStack = { language: 'Go', framework: 'Unknown' };
    try {
  ...
  private async detectPackageManager(): Promise<string> {
    ...
    if (await fileExists(path.join(root, 'Pipfile.lock'))) return 'Pipenv';
    if (await fileExists(path.join(root, 'uv.lock'))) return 'uv';
    if (await fileExists(path.join(root, 'requirements.txt'))) return 'pip';

    // Java 构建工具
    if (await fileExists(path.join(root, 'pom.xml'))) return 'Maven';
    if (await fileExists(path.join(root, 'build.gradle')) || await fileExists(path.join(root, 'build.gradle.kts'))) return 'Gradle';

    // Go
    if (await fileExists(path.join(root, 'go.mod'))) return 'Go Modules';
  ...
  private async detectBuildTool(): Promise<string> {
    ...
    // Python
    if (await fileExists(path.join(root, 'Makefile'))) return 'Make';
