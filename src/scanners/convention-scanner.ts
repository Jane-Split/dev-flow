  ...
  private async inferNamingConventions(): Promise<CodingConvention[]> {
    ...
    try {
      ...
      if (!srcExists) {
        // 没有src目录，返回默认规范
        conventions.push(
          { id: 'naming-component', category: 'naming', name: 'Component naming', description: 'Components should be named in PascalCase', examples: ['Button', 'UserProfile'], severity: 'warn' as const },
          { id: 'naming-function', category: 'naming', name: 'Function naming', description: 'Functions should be named in camelCase', examples: ['handleSubmit', 'fetchData'], severity: 'warn' as const },
        );
        return conventions;
      }
      ...
      for (const file of codeFiles.slice(0, 50)) { // 最多扫描50个文件
        ...
        for (const m of exportMatches) pascalCaseCount++;
        ...
        for (const m of funcMatches) {
          totalFunctions++;
          const name = m[1];
          if (/^[A-Z]/.test(name) && !/^[A-Z]+$/.test(name)) pascalCaseCount++;
          else if (/^[a-z]/.test(name) && name.includes('_')) snakeCaseCount++;
          else if (/^[a-z]/.test(name)) camelCaseCount++;
        }

        // 检测 use 前缀（React Hooks）
        if (content.match(/(?:const|function)\s+use[A-Z]/)) hasUsePrefix = true;
      }
