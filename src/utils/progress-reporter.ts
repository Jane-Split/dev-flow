...
export class ProgressReporter {
  ...
  updateStage(
    ...
    if (
      ...
      stage.duration = Math.round(
        (new Date(now).getTime() - new Date(stage.startTime).getTime()) / 1000
      );
    }
  ...
  updateTask(
    ...
    task.status = status;
    if (expert !== undefined) task.expert = expert;
  ...
  render(): string {
    ...
    const completedItems = this.report.stages.filter(
   ...
   * @returns 保存的文件路径
   */
  async save(projectRoot: string): Promise<string> {
    const content = this.render();
    const filePath = path.join(
      projectRoot,
      '.dev-flow',
      'sessions',
      `progress-${this.report.sessionId}.md`
    );
   ...
   */
  getProgress(): number {
    if (this.report.stages.length === 0) return 0;
  ...
  getSummary(): string {
    ...
    const completedStages = this.report.stages.filter(
      s => s.status === 'completed' || s.status === 'skipped'
    ).length;
   ...
   * 获取阶段状态图标
   */
  private getStageStatusIcon(status: StageProgress['status']): string {
    switch (status) {
   ...
   * 获取阶段状态文本
   */
  private getStageStatusText(status: StageProgress['status']): string {
    switch (status) {
   ...
   * 获取任务状态图标
   */
  private getTaskStatusIcon(status: TaskProgress['status']): string {
    switch (status) {
      case 'completed': return '\u2705';
      case 'running': return '\U0001f504';
      case 'failed': return '\u274c';
   ...
   * 获取任务状态文本
   */
  private getTaskStatusText(status: TaskProgress['status']): string {
    switch (status) {
      case 'completed': return '完成';
      case 'running': return '执行中';
      case 'failed': return '失败';
  ...
  private checkOverallStatus(): void {
    if (this.report.stages.length === 0) return;
    ...
    // 如果所有阶段都已完成或跳过
    const allDone = this.report.stages.every(
      s => s.status === 'completed' || s.status === 'skipped'
    );
    if (allDone) {
