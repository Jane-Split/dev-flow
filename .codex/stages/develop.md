...
> **所有代码编辑必须由 develop-expert subagent 执行。** 完整规则见 `.claude/references/protocol.md`。
...
> **详细分段执行规则见 `.claude/agents/develop-expert.md` — 结构化代码分段生成协议章节。**
...
```
Step D1: 读取阶段指令 → Read .claude/stages/develop.md（本文件）
...
> **完整执行规范见 `.claude/agents/backend-develop-expert.md` 和 `.claude/agents/frontend-develop-expert.md`。**
...
> **完整禁止事项和完整性铁律见 `.claude/agents/backend-develop-expert.md` 和 `.claude/agents/frontend-develop-expert.md`。**
