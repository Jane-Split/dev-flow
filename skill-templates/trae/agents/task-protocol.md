...
context_injection:
  brief_file: ".dev-flow/runtime/task-brief-{taskId}.md"
  auto_prepared: true
  brief_includes:
    - "任务描述和子任务设计"
    - "Design Contract 相关定义"
    - "依赖类实际代码"
    - "编码规范"
    - "前置任务产出"
  note: "如果此文件存在，subagent 应优先读取获取上下文，无需自行读取多个文件"
...
validation:
  auto_validated: true  # Orchestrator 自动运行 validate-result.cjs
  validation_script: "node scripts/validate-result.cjs --task {taskId} --demand {demandName}"
  validation_status: "pending"  # pending | passed | failed

next_tasks_hint: [string]   # 建议的后续任务
```

### 4.3 Context Injection Protocol (上下文注入协议)

> **v3.0 新增**：为确保 subagent 上下文完整性和可靠性，Orchestrator 在派发前自动注入上下文。

**注入流程**：

```
Orchestrator 派发 subagent 前:
  1. 运行: node scripts/prepare-context.cjs --task {taskId} --demand {demandName}
  2. 确认 .dev-flow/runtime/task-brief-{taskId}.md 已生成
  3. 在派发命令中指示 subagent 优先读取 task-brief.md

Subagent 启动后:
  1. 检查 .dev-flow/runtime/task-brief-{taskId}.md 是否存在
  2. 存在 → 读取该文件获取完整上下文，直接开始工作
  3. 不存在 → 按原有流程自行读取依赖文件（降级模式）
```
...
2. Follow the Design Contract exactly
...
## Subtask Design
{subtask design content}

## Design Contract (Relevant)
{relevant contract definitions}

## Develop Rules (Core)
{core development rules - prohibitions, completeness requirements}
...
validation_protocol:
  pre_dispatch:
    script: "scripts/prepare-context.cjs"
    action: "生成 task-brief-{taskId}.md"

  post_completion:
    script: "scripts/validate-result.cjs"
    action: "校验 task-result-{taskId}.yaml"
    checks:
      - file_existence: "completed_files 中每个文件都存在且非空"
      - no_todo: "无 TODO/FIXME/空方法体"
      - no_log_only: "无仅含日志的方法体"
      - contract_match: "Design Contract 方法签名一致"
