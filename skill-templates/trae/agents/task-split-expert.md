...
> **本 Expert 只负责任务拆分，不负责项目扫描。**
>
> 所有项目结构信息必须通过以下方式获取：
> - 从 `design-contract.yaml` 获取设计信息
> - 从 `design-result.md` 获取详细设计
> - **禁止执行 Glob/Read 对项目源码进行扫描**
> - **禁止重新扫描 .dev-flow/memory/ 目录**
>
> **原因**：项目扫描已在 Research 阶段完成并写入 memory。Task Split 阶段只需要分析设计文档中的依赖关系。
