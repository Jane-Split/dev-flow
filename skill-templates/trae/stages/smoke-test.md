...
> **所有文件编辑必须由 smoke-test-expert subagent 执行。**
...
**交付物路径**：`.dev-flow/deliverables/08-smoke-test-report.md`

**交付物内容**：
```markdown
<!-- @generated-by: smoke-test-expert subagent | session: {session-id} | stage: smoke-test -->
...
**自检**：
- 交付物文件已生成且内容非空
- 包含 `@generated-by: smoke-test-expert subagent` 溯源注释
- 核心流程测试结果完整
- 问题已记录
...
| # | 确认项 | 状态 |
|---|--------|------|
| 0 | **执行者审计**：本阶段由 smoke-test-expert subagent 执行，主 Agent 未直接编辑任何文件 | ⬜ 待确认 |
