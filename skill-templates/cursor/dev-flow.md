...
> **完整规则见 `.cursor/references/protocol.md` — 零编辑铁律 v2.0 章节。**
...
> **完整规则见 `.cursor/references/protocol.md` — Subagent 失败硬阻断规则章节。**
...
> **完整规则见 `.cursor/references/protocol.md`，包含：阶段交付物协议、确认持久化规则、确认文件格式、门禁检查流程、确认 Checklist 模板、阶段历史压缩规则。**
...
| Clarify（需求澄清） | `.cursor/stages/clarify.md` | 进入阶段二 | `{需求简称}/research.confirmed`（可选阶段，可跳过） |
| Analyze（需求分析） | `.cursor/stages/analyze.md` | 进入阶段三 | `{需求简称}/research.confirmed`（最低要求）或 `{需求简称}/clarify.confirmed`（如 Clarify 已执行） |
| Design（详细设计） | `.cursor/stages/design.md` | 进入阶段四 | `{需求简称}/analyze.confirmed` |
| Task Split（任务拆分） | `.cursor/stages/task-split.md` | 进入阶段五 | `{需求简称}/design.confirmed` |
| **Develop（开发执行）** | **`.cursor/stages/develop.md`** | **进入阶段六** | `{需求简称}/task-split.confirmed` |
| Test（统一测试） | `.cursor/stages/test.md` | 进入阶段七 | `{需求简称}/develop.confirmed` |
| Fix（Bug 修复） | `.cursor/stages/fix.md` | 进入阶段八 | 无（Bug 触发） |
| Hotfix（独立模式） | `.cursor/stages/hotfix.md` | 使用 Hotfix 模式 | 无 |
| Delivery（交付报告） | `.cursor/stages/delivery.md` | 进入阶段九 | `{需求简称}/test.confirmed` |
| E2E 验证（独立） | `.cursor/stages/test.md` | 使用 -e2e 命令 | `{需求简称}/develop.confirmed` |
| UI 验证（独立） | `.cursor/stages/test.md` | 使用 -e2e-ui 命令 | `{需求简称}/develop.confirmed` |
| API+DB 验证（独立） | `.cursor/stages/test.md` | 使用 -e2e-api 命令 | `{需求简称}/develop.confirmed` |
| 完整验证闭环 | `.cursor/stages/test.md` | 使用 -verify 命令 | `{需求简称}/develop.confirmed` |

### 加载规则

1. **标准模式/企业级模式**：按顺序进入每个阶段时，读取对应阶段的指令文件
2. **Subagent 执行**：每个 subagent 只加载自己阶段的指令文件
3. **Develop 阶段额外加载**：进入 Develop 阶段时，还需加载 `.cursor/stages/code-reference.md`（包含代码模板和错误模式）
...
> 详细记忆目录结构、使用规则、文件格式和会话/长期记忆分类见 `.cursor/references/memory-system.md`。
...
> 详细学习机制、示例和效果评估见 `.cursor/references/learning-system.md`。
