...
| Integration Test（集成测试） | `.cursor/stages/integration-test.md` | 进入阶段九 |
| Delivery（交付报告） | `.cursor/stages/delivery.md` | 进入阶段十 |
...
```
Step 1: 读取阶段指令 → Read .cursor/stages/research.md
Step 2: 执行 Research 阶段 → 扫描项目，生成 memory/
Step 3: 暂停 → 展示调研结果，等待用户确认
  ↓ 用户确认
Step 4: 读取阶段指令 → Read .cursor/stages/analyze.md
Step 5: 执行 Analyze 阶段 → 分析需求，输出分析文档
Step 6: 暂停 → 展示分析结果，等待用户确认
  ↓ 用户确认
Step 7: 读取阶段指令 → Read .cursor/stages/design.md
