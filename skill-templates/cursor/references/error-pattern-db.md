...
## 前端错误模式（v3.7.0 新增，适用 frontend/fullstack/java-fullstack）

### 组件渲染错误
| 错误模式 | 症状 | 常见原因 | 修复策略 |
|---------|------|---------|---------|
| 空白页面 | 页面无内容渲染 | 路由配置错误、组件导出缺失 | 检查路由配置、验证组件 default export |
| 白屏错误 | React/Vue 崩溃 | JSX语法错误、Hook规则违反 | 检查浏览器控制台堆栈 |
| 样式丢失 | 元素无样式 | CSS模块路径错误、Tailwind未配置 | 检查import路径、验证框架配置 |

### 状态管理错误
| 错误模式 | 症状 | 常见原因 | 修复策略 |
|---------|------|---------|---------|
| 状态未更新 | UI未响应数据变化 | 直接修改state、未调用setState | 使用不可变更新模式 |
| 无限循环 | 页面卡死 | useEffect依赖项错误 | 检查依赖数组 |

### API 调用错误
| 错误模式 | 症状 | 常见原因 | 修复策略 |
|---------|------|---------|---------|
| CORS错误 | API请求被拦截 | 后端未配置CORS | 配置后端允许跨域 |
| 401/403 | 请求被拒绝 | Token过期/缺失 | 检查认证中间件、Token刷新 |
