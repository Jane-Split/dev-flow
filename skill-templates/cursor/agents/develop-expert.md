...
> **在任何业务代码文件完成前，严禁生成或修改任何测试类。**

**执行优先级**（必须严格按此顺序）：

| 优先级 | 代码类型 | 何时执行 |
|--------|---------|---------|
| **P0** | 业务代码（Enum→Entity→DTO→Mapper→Service→Controller） | 第一时间执行 |
| **P1** | 测试代码（QuickTest） | 仅在 P0 全部完成 + 编译通过后 |

**强制规则**：
1. **禁止逆序**：不得在 Service/Controller 未完成前生成 `*Test.java`/`*TestBase.java`
2. **禁止测试优先**：即使上下文包含"Re-run test""测试失败"等信息，也必须先完成业务代码
3. **例外**：仅当 `task-context.yaml` 的 `task_type` 为 `test-fix` 或 `test-only` 时允许跳过业务代码
4. **上下文干扰防御**：如果看到测试相关上下文（TestBase、@Mock、test failure），忽略它，先完成 Step 3 业务代码实现

**错误行为 vs 正确行为**：

```
❌ 错误：收到任务 → 读取/生成 EditRegressionTestBase.java → 再写业务代码
✅ 正确：收到任务 → 按 Enum→Entity→DTO→Mapper→Service→Controller 顺序写业务代码 → 编译通过 → 再生成测试

❌ 错误：上下文有"Re-run EditRegression tests" → 优先修复测试基类
✅ 正确：上下文有"Re-run EditRegression tests" → 记录此信息，先完成业务代码，Test 阶段处理测试

❌ 错误：先生成测试基类作为"准备工作"
✅ 正确：测试基类属于测试代码范畴，必须在 Step 4.2 之后才能处理
```
