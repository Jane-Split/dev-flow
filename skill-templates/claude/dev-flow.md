...
| 阶段 | 指令文件 | 加载时机 |
|------|---------|---------|
| Research（项目调研） | `.claude/stages/research.md` | 进入阶段一 |
| Analyze（需求分析） | `.claude/stages/analyze.md` | 进入阶段二 |
| Design（详细设计） | `.claude/stages/design.md` | 进入阶段三 |
| Task Split（任务拆分） | `.claude/stages/task-split.md` | 进入阶段四 |
| **Develop（开发执行）** | **`.claude/stages/develop.md`** | **进入阶段五** |
| Unit Test（单元测试） | `.claude/stages/unit-test.md` | 进入阶段六 |
| Fix（Bug 修复） | `.claude/stages/fix.md` | 进入阶段七 |
| Hotfix（独立模式） | `.claude/stages/hotfix.md` | 使用 Hotfix 模式 |
| Smoke Test（冒烟测试） | `.claude/stages/smoke-test.md` | 进入阶段八 |
| E2E Test（端到端测试） | `.claude/stages/e2e-test.md` | 进入阶段九 |
| Integration Test（集成测试） | `.claude/stages/integration-test.md` | 进入阶段十 |
| Delivery（交付报告） | `.claude/stages/delivery.md` | 进入阶段十一 |
