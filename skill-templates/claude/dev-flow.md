...
> **详细审计规范、@generated-by 溯源格式、audit-log.yaml 格式** → 详见 `.claude/references/zero-edit-audit.md`
...
> 📄 **详细内容（溯源格式、审计流程、audit-log.yaml 格式）** → 详见 `.claude/references/zero-edit-audit.md`

### 🔴🔴 Subagent 失败硬阻断规则（最高优先级，不可违反）

> **核心原则**：Subagent 执行失败后，主 Agent **绝对禁止**直接介入执行任务。
> 必须遵循三级失败处理协议，逐步升级，最终由人工介入。
> **详细决策树、Level 2 诊断报告模板、Level 3 升级报告模板** → 详见 `.claude/references/failure-decision-tree.md`
...
> 📄 **完整决策树、诊断报告模板、升级报告模板、subagent-failures.yaml 格式** → 详见 `.claude/references/failure-decision-tree.md`
...
> 详见 `.claude/stages/research.md`（Research 阶段指令文件）。
...
| Analyze（需求分析） | `.claude/stages/analyze.md` | 进入阶段二 | `research.confirmed` |
| Design（详细设计） | `.claude/stages/design.md` | 进入阶段三 | `analyze.confirmed` |
| Task Split（任务拆分） | `.claude/stages/task-split.md` | 进入阶段四 | `design.confirmed` |
| **Develop（开发执行）** | **`.claude/stages/develop.md`** | **进入阶段五** | `task-split.confirmed` |
| Unit Test（单元测试） | `.claude/stages/unit-test.md` | 进入阶段六 | `develop.confirmed` |
| Fix（Bug 修复） | `.claude/stages/fix.md` | 进入阶段七 | 无（Bug 触发） |
| Hotfix（独立模式） | `.claude/stages/hotfix.md` | 使用 Hotfix 模式 | 无 |
| Smoke Test（冒烟测试） | `.claude/stages/smoke-test.md` | 进入阶段八 | `unit-test.confirmed` |
| E2E Test（端到端测试） | `.claude/stages/e2e-test.md` | 进入阶段九 | `smoke-test.confirmed` |
| Integration Test（集成测试） | `.claude/stages/integration-test.md` | 进入阶段十 | `e2e-test.confirmed` |
| Delivery（交付报告） | `.claude/stages/delivery.md` | 进入阶段十一 | `integration-test.confirmed` |
