...
> **完整规则见 `.qoder/references/protocol.md` — 零编辑铁律 v2.0 章节。**
...
> **完整规则见 `.qoder/references/protocol.md` — Subagent 失败硬阻断规则章节。**
...
> **完整规则见 `.qoder/references/protocol.md`，包含：阶段交付物协议、确认持久化规则、确认文件格式、门禁检查流程、确认 Checklist 模板、阶段历史压缩规则。**
...
| 阶段 | 指令文件 | 加载时机 | 前置确认文件 |
|------|---------|---------|------------|
| Research（项目调研） | `.qoder/stages/research.md` | 进入阶段一 | 无 |
| Clarify（需求澄清） | `.qoder/stages/clarify.md` | 进入阶段二 | `{需求简称}/research.confirmed`（可选阶段，可跳过） |
| Analyze（需求分析） | `.qoder/stages/analyze.md` | 进入阶段三 | `{需求简称}/research.confirmed`（最低要求）或 `{需求简称}/clarify.confirmed`（如 Clarify 已执行） |
| Design（详细设计） | `.qoder/stages/design.md` | 进入阶段四 | `{需求简称}/analyze.confirmed` |
| Task Split（任务拆分） | `.qoder/stages/task-split.md` | 进入阶段五 | `{需求简称}/design.confirmed` |
| **Develop（开发执行）** | **`.qoder/stages/develop.md`** | **进入阶段六** | `{需求简称}/task-split.confirmed` |
| Test（统一测试） | `.qoder/stages/test.md` | 进入阶段七 | `{需求简称}/develop.confirmed` |
| Fix（Bug 修复） | `.qoder/stages/fix.md` | 进入阶段八 | 无（Bug 触发） |
| Hotfix（独立模式） | `.qoder/stages/hotfix.md` | 使用 Hotfix 模式 | 无 |
| Delivery（交付报告） | `.qoder/stages/delivery.md` | 进入阶段九 | `{需求简称}/test.confirmed` |
| E2E 验证（独立） | `.qoder/stages/test.md` | 使用 -e2e 命令 | `{需求简称}/develop.confirmed` |
| UI 验证（独立） | `.qoder/stages/test.md` | 使用 -e2e-ui 命令 | `{需求简称}/develop.confirmed` |
| API+DB 验证（独立） | `.qoder/stages/test.md` | 使用 -e2e-api 命令 | `{需求简称}/develop.confirmed` |
| 完整验证闭环 | `.qoder/stages/test.md` | 使用 -verify 命令 | `{需求简称}/develop.confirmed` |
