...
> **所有代码编辑必须由 develop-expert subagent 执行。** 完整规则见 `{{REFERENCES_PATH}}protocol.md`。
...
> **详细分段执行规则见 `{{AGENTS_PATH}}develop-expert.md` — 结构化代码分段生成协议章节。**
...
> **完整执行规范见 `{{AGENTS_PATH}}backend-develop-expert.md` 和 `{{AGENTS_PATH}}frontend-develop-expert.md`。**
...
> **完整禁止事项和完整性铁律见 `{{AGENTS_PATH}}backend-develop-expert.md` 和 `{{AGENTS_PATH}}frontend-develop-expert.md`。**
