...
> **完整规则见 `{{REFERENCES_PATH}}protocol.md` — 零编辑铁律 v2.0 章节。**
...
> **完整规则见 `{{REFERENCES_PATH}}protocol.md` — Subagent 失败硬阻断规则章节。**
...
> **完整规则见 `{{REFERENCES_PATH}}protocol.md`，包含：阶段交付物协议、确认持久化规则、确认文件格式、门禁检查流程、确认 Checklist 模板、阶段历史压缩规则。**
...
> **完整门禁检查流程见 `{{REFERENCES_PATH}}protocol.md` — 阶段门禁检查章节。**
...
| Clarify（需求澄清） | `{{STAGES_PATH}}clarify.md` | 进入阶段二 | `{需求简称}/research.confirmed`（可选阶段，可跳过） |
| Analyze（需求分析） | `{{STAGES_PATH}}analyze.md` | 进入阶段三 | `{需求简称}/research.confirmed`（最低要求）或 `{需求简称}/clarify.confirmed`（如 Clarify 已执行） |
| Design（详细设计） | `{{STAGES_PATH}}design.md` | 进入阶段四 | `{需求简称}/analyze.confirmed` |
| Task Split（任务拆分） | `{{STAGES_PATH}}task-split.md` | 进入阶段五 | `{需求简称}/design.confirmed` |
| **Develop（开发执行）** | **`{{STAGES_PATH}}develop.md`** | **进入阶段六** | `{需求简称}/task-split.confirmed` |
| Test（统一测试） | `{{STAGES_PATH}}test.md` | 进入阶段七 | `{需求简称}/develop.confirmed` |
| Fix（Bug 修复） | `{{STAGES_PATH}}fix.md` | 进入阶段八 | 无（Bug 触发） |
| Hotfix（独立模式） | `{{STAGES_PATH}}hotfix.md` | 使用 Hotfix 模式 | 无 |
| Delivery（交付报告） | `{{STAGES_PATH}}delivery.md` | 进入阶段九 | `{需求简称}/test.confirmed` |
