...
| Integration Test（集成测试） | `{{STAGES_PATH}}integration-test.md` | 进入阶段九 |
| Delivery（交付报告） | `{{STAGES_PATH}}delivery.md` | 进入阶段十 |
...
```
Step 1: 读取阶段指令 → Read {{STAGES_PATH}}research.md
Step 2: 执行 Research 阶段 → 扫描项目，生成 memory/
Step 3: 暂停 → 展示调研结果，等待用户确认
  ↓ 用户确认
Step 4: 读取阶段指令 → Read {{STAGES_PATH}}analyze.md
Step 5: 执行 Analyze 阶段 → 分析需求，输出分析文档
Step 6: 暂停 → 展示分析结果，等待用户确认
  ↓ 用户确认
Step 7: 读取阶段指令 → Read {{STAGES_PATH}}design.md
Step 8: 执行 Design 阶段 → 详细设计，输出设计文档
Step 9: 暂停 → 展示设计方案，等待用户确认
  ↓ 用户确认
Step 10: 读取阶段指令 → Read {{STAGES_PATH}}task-split.md
Step 11: 执行 Task Split → 拆分任务，输出任务清单
Step 12: 暂停 → 展示任务清单，等待用户确认
  ↓ 用户确认
Step 13: 读取阶段指令 → Read {{STAGES_PATH}}develop.md
        读取代码参考 → Read {{STAGES_PATH}}code-reference.md
Step 14: 执行 Develop 阶段 → 编写完整代码
Step 15: 暂停 → 展示开发结果，等待用户确认
  ↓ 用户确认
Step 16: 读取阶段指令 → Read {{STAGES_PATH}}unit-test.md
