...
- [14. v1.0.5 新特性](#14-v105-新特性)
- [15. 执行模式选择指南](#15-执行模式选择指南)
- [16. 常见问题](#16-常见问题)
...
### 14.1 上下文智能管理（Context Manager）

**解决的问题**：上下文限制导致 AI 敷衍生成、代码不完整、用日志占位替代业务逻辑

**核心原则**：

> **准确性优先，宁可串行也不生成错误代码**

### 14.2 最小安全上下文阈值（50KB 硬约束）

每个 develop-expert subagent 至少分配 50KB 上下文，这是硬约束不可突破：

| 组件 | 分配 |
|------|------|
| 系统指令（develop-expert.md） | 10KB |
| 编码规范（conventions.md） | 5KB |
| 设计文档 | 15KB |
| Step 2.5 依赖类读取 | 15KB |
| 代码生成空间 | 10KB |
| 应急缓冲 | 5KB |
...
| 任务类型 | 依赖特点 | 执行模式 | 上下文需求 |
|---------|---------|---------|-----------|
| Entity | 无依赖 | 可并行 | 10-20KB |
| Enum | 无依赖 | 可并行 | 10-20KB |
| DTO | 只读依赖 | 可并行（通过契约） | 20-30KB |
| Mapper | 只读依赖 | 可并行（通过契约） | 20-30KB |
| Service | 写依赖 | **必须串行** | 40-60KB |
| Controller | 写依赖 | **必须串行** | 40-60KB |
...
```
Segment 1: 完成 Step 2.5 强制读取验证（25KB）
    ↓ 文件系统传递状态
Segment 2: 完成代码生成（第一部分）（25KB）
    ↓ 文件系统传递状态
Segment 3: 完成代码生成（第二部分）（25KB）
```

每个 segment 在干净上下文中执行，通过文件系统传递状态，不累积上下文。

### 14.7 串行执行兜底（Safety Net）

当以下条件触发时，自动降级为串行执行：

| 触发条件 | 优先级 | 动作 |
|---------|--------|------|
| 可用上下文 < 50KB | CRITICAL | 强制串行 |
| 任务复杂度高且预估 > 70KB | HIGH | 建议串行 |
| Step 2.5 多次失败 | MEDIUM | 降级串行 |

串行执行顺序：`Entity → Enum → DTO → Mapper → Service → Controller`

### 14.8 用户价值

- **准确性保证**：每个任务有充足上下文，不会敷衍生成
- **自动降级**：上下文不足时自动切换为串行，无需人工干预
- **进度保护**：分段执行确保长任务不会中断丢失
- **效率平衡**：在保证准确性的前提下最大化并行效率

## 15. 执行模式选择指南

### 15.1 三种执行模式

dev-flow 支持三种执行模式，系统会自动选择最优模式，也支持手动指定：

| 模式 | 特点 | 适用场景 |
|------|------|---------|
| **并行模式** | 多个 subagent 同时执行，效率最高 | 小型项目、简单需求 |
| **串行模式** | 任务逐个执行，准确性最高 | 大型项目、复杂需求、关键业务 |
| **混合模式** | 无依赖任务并行 + 依赖链任务串行 | 中型项目、平衡效率与准确性 |
...
#### 方法一：使用 `-extreme` 参数（完全串行）

```bash
# 极端模式：每个文件一个 agent，完全串行执行
/dev-flow -subagent -extreme <需求描述>
```

**特点**：
- 每个文件由独立的 develop-expert subagent 生成
- 所有任务串行执行，每个任务在干净上下文中启动
- 准确性最高，但执行时间较长
...
#### 方法二：配置 `max_parallel_tasks: 1`

在 `.dev-flow/config.yaml` 中配置：

```yaml
# 强制串行执行
execution:
  max_parallel_tasks: 1  # 设为 1 即等效于串行
```
...
| 场景 | 推荐模式 | 配置方式 |
|------|---------|---------|
| 小型项目（<10 个类） | 并行模式 | 默认配置 |
| 中型项目（10-30 个类） | 自动模式 | 默认配置 |
| 大型项目（>30 个类） | 串行模式 | `-extreme` 或 `max_parallel_tasks: 1` |
| 关键业务系统 | 串行模式 | `-extreme` |
| 上下文受限（<100KB） | 串行模式 | 系统自动切换 |
...
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  任务队列：[Entity, Enum, DTO, Mapper, Service, Controller] │
│                                                              │
│  Step 1: 执行 Entity                                         │
│          ├── 启动 develop-expert（干净上下文 50KB）          │
│          ├── 完成 Step 2.5 强制读取验证                      │
│          ├── 生成代码并写入文件                              │
│          └── 清理上下文，保存状态到文件系统                   │
│                         ↓                                    │
...
│  ... 继续执行后续任务 ...                                    │
│                         ↓                                    │
│  Step N: 执行 Controller                                     │
│          ├── 启动 develop-expert（干净上下文 60KB）          │
│          ├── 读取所有依赖定义（通过文件系统）                │
│          ├── 生成代码并写入文件                              │
│          └── 完成，输出最终报告                              │
│                                                              │
...
|------|---------|---------|
| **执行效率** | 高（2-3x 提升） | 较低 |
| **准确性** | 依赖上下文充足 | 最高（100% 保证） |
| **上下文需求** | 总上下文 = 任务数 × 50KB | 每任务 50KB |
| **适用项目规模** | 小型（<10 个类） | 大型（>30 个类） |
| **代码完整性** | 依赖上下文管理 | 完全保证 |
| **错误率** | 低（有 Step Enforcer 保护） | 极低 |
...
A: 串行模式的执行时间约为并行模式的 2-3 倍，但可以保证 100% 的代码正确性和完整性。对于关键业务系统，这个代价是值得的。

**Q: 可以在执行过程中切换模式吗？**

A: 不建议。系统会在任务开始前根据上下文情况自动选择最优模式。如果执行过程中发现上下文不足，会自动触发分段执行机制。

**Q: 如何判断当前使用的是什么模式？**

A: 在执行开始时，系统会输出执行模式决策报告：
