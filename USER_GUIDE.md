...
- [6. Subagent 模式](#6-subagent-模式)
  ...
  - [6.7 精准按需加载](#67-精准按需加载)
  - [6.8 与标准模式的对比](#68-与标准模式的对比)
...
4. **Develop** → AI 按依赖顺序生成代码：Enum → Entity → DTO → Mapper → Service → Controller → 你确认
5. **Test** → AI 生成 JUnit 5 + Mockito 测试（Controller/Service/Mapper 分层测试）→ 你确认
6. **Fix** → 如有失败用例，AI 自动修复（最多 3 轮）
...
3. **Design 阶段**：design-expert 基于分析结果进行详细设计
4. **Develop 阶段**：orchestrator 根据 DAG 依赖图进行拓扑排序，分批启动 develop-expert
   - 无依赖的任务并行执行（如不同服务的开发任务）
   - 有依赖的任务串行执行（如 Entity → DTO → Service → Controller）
5. **Verify 阶段**：verify-expert 验证所有生成代码的质量和完整性
...
| 任务拆分 | 无 | DAG 依赖图 + 拓扑排序 |
| 代码生成 | 主 agent 直接生成 | develop-expert 并行生成 |
| 效率 | 适合小任务 | 复杂任务效率翻倍 |
...
1. 安装：`npx dev-flow codex`（将 AGENTS.md 复制到项目根目录）
2. 启动：在终端运行 `codex`
3. 使用自然语言：`请执行 dev-flow research` 或 `请执行 dev-flow 全流程，实现用户登录功能`

Codex 使用 AGENTS.md 作为项目指令文件，与 dev-flow 的 SKILL.md 功能等价。
