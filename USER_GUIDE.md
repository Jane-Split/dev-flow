...
Subagent 模式是 dev-flow 的高级运行模式，将复杂任务拆分为多个独立的子代理并行执行，大幅提高开发效率。

**两种子代理模式**：
- **标准模式**：串行执行子代理（一个完成后下一个才开始）
- **企业级模式**（`-subagent` 参数）：并行执行子代理（多个同时执行）

### 6.2 适用场景

| 场景 | 推荐模式 | 原因 |
|------|---------|------|
| 小型需求（< 5 个文件） | 标准模式 | 并行开销 > 收益 |
| 中型需求（5-20 个文件） | 标准模式 或 企业级模式 | 根据任务数动态评估 |
| 大型需求（> 20 个文件） | 企业级模式 | 并行大幅提升效率 |
| 多服务/多模块 | 企业级模式 | 天然适合并行 |
| 有写写冲突 | 标准模式（自动降级） | 冲突任务必须串行 |
