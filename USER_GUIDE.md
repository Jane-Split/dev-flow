...
- **五层防御体系（v4.0.0）**：L1 动态上下文预算 → L2 Checkpoint 系统 → L3 完整性门控 → L4 冗余验证链 → L5 故障自动恢复，保障代码生成质量与可靠性
...
Develop 阶段集成五层防御体系，从代码生成前到生成后全链路保障质量：

- **L1 动态上下文预算**：task-brief 生成前自动计算上下文预算，支持 8+ 模型（GPT-4o / Claude 3.5 Sonnet / Gemini Pro 等），根据模型上下文窗口动态分配 token 配额，防止子代理上下文溢出
- **L2 Checkpoint 系统**：代码生成过程中自动创建状态快照，失败时可回滚到上一个稳定状态；通过方法依赖图拓扑排序确定最优方法填充顺序，减少编译依赖阻塞
- **L3 完整性门控**：subagent 派发前执行 3 项硬检查——截断检测（输出是否被截断）、缺失检测（必需文件是否遗漏）、契约一致性检测（接口契约是否匹配），任一检查不通过则阻止派发
- **L4 冗余验证链**：静态验证套件执行 6 项自动验证（文件存在性、非空检查、TODO/FIXME 检测、空方法体检测、log-only 方法体检测、return null 检测）+ 独立验证工作进程（contract-validator）执行 R1-R5 规则校验 + R5 逻辑覆盖率自动化验证（强制 100%）
- **L5 故障自动恢复**：内置降级策略矩阵（根据错误类型自动选择重试/跳过/人工升级策略）、部分交付报告（失败任务不影响其他任务交付）、编译循环管理（自动解析编译错误并修复，最多 3 轮）
...
3. **Develop 阶段**：按 DAG 顺序派发子代理
   - 标准模式：逐批次串行
   - 企业级模式：同批次并行
4. **验证链（L4 冗余验证链）**：每个子代理完成后，执行三层冗余验证
   - **Step 5.1：develop-expert 开发自检**
     - 执行 Step 4.3 设计→代码逻辑回溯验证
     - 从 design-contract.yaml 提取所有逻辑单元，在代码中逐条定位实现
     - 计算逻辑覆盖率，必须达到 100%
   - **Step 5.2：contract-validator 独立验证（R1-R5 规则校验）**
     - **R1 接口契约一致性**：验证子任务实现与 design-contract.yaml 中的接口定义完全匹配（方法名、参数类型、返回值、异常声明）
     - **R2 数据契约一致性**：验证 DTO/Entity/VO 字段类型、约束、默认值与设计一致
     - **R3 事件契约一致性**：验证事件发布/订阅的 topic、payload schema、顺序与设计一致
     - **R4 依赖契约一致性**：验证跨子任务调用的服务依赖、API 调用路径与 interface-registry.yaml 一致
     - **R5 逻辑覆盖率验证（阻塞级）**：强制 100% 覆盖率，从设计文档提取的逻辑单元必须在代码中全部找到对应实现；未达标则阻塞进入下一阶段
   - **Step 5.3：verify-expert 质量检查**
     - 编译验证（Java: `mvn compile` / 前端: `tsc --noEmit`）
     - 静态验证套件 6 项检查（文件存在性、非空、TODO/FIXME、空方法体、log-only、return null）
     - 代码质量评分（复杂度、重复度、规范符合度）
...
五层防御体系是 dev-flow v4.0.0 引入的全链路质量保障机制，覆盖代码生成前、生成中、生成后的每个关键环节。五层之间层层递进，形成纵深防御。

### 12.1 体系概览

```text
L1 动态上下文预算  →  预防上下文溢出
L2 Checkpoint 系统  →  状态快照与回滚
L3 完整性门控      →  派发前硬检查
L4 冗余验证链      →  生成后多重验证
L5 故障自动恢复    →  异常降级与恢复
```
...
**目标**：防止子代理因上下文过长导致输出截断或质量下降。

**工作原理**：

1. **模型感知**：内置 8+ 主流模型的上下文窗口配置（GPT-4o、Claude 3.5 Sonnet、Gemini Pro、Claude 3 Opus 等）
2. **自动计算**：task-brief 生成前，根据当前任务复杂度、依赖文件数量、设计文档大小自动计算所需 token 数
3. **动态配额**：为子代理预留充足上下文空间，brief 大小上限 120KB（可根据模型自动调整）
4. **超限预警**：当预估 token 超过模型容量 80% 时，自动触发任务拆分或历史压缩

**使用方式**：无需手动配置，系统自动执行。

### 12.3 L2 Checkpoint 系统

**目标**：代码生成过程中保存状态快照，失败时可回滚，减少重复工作。

**工作原理**：

1. **自动快照**：每完成一个方法填充或一个文件生成后，自动创建 checkpoint
2. **依赖图排序**：构建方法级依赖图，通过拓扑排序确定最优填充顺序
   - 无依赖的方法优先填充
   - 被依赖的方法完成后，再填充依赖方
   - 减少编译阻塞和循环依赖风险
3. **失败回滚**：当某个方法填充失败时，可回滚到上一个 checkpoint，而非重新生成整个文件
4. **增量恢复**：从 checkpoint 恢复后，仅重新执行失败部分，已成功的部分跳过

**使用方式**：无需手动配置，系统自动执行。

### 12.4 L3 完整性门控

**目标**：subagent 派发前执行硬检查，阻止不完整或有问题的任务进入执行阶段。

**三项硬检查**：

| 检查项 | 说明 | 失败处理 |
|--------|------|----------|
| **截断检测** | 检测 task-brief 或设计文档是否在生成过程中被截断 | 拒绝派发，返回重新生成 |
| **缺失检测** | 检测必需文件（设计契约、接口定义、依赖类）是否全部存在 | 拒绝派发，补充缺失文件 |
| **契约一致性检测** | 检测子任务设计与全局 design-contract.yaml 的接口契约是否匹配 | 拒绝派发，修正契约冲突 |

**使用方式**：无需手动配置，系统自动执行。任一检查不通过会在主 Agent 日志中输出具体原因。

### 12.5 L4 冗余验证链

**目标**：代码生成后通过多重独立验证，确保产出质量。
...
| 文件存在性 | 所有预期文件是否已生成 |
| 非空检查 | 文件内容是否非空 |
| TODO/FIXME 检测 | 是否存在未完成的 TODO 或 FIXME 标记 |
| 空方法体检测 | 是否存在只有方法签名没有实现体的方法 |
| log-only 方法体检测 | 是否存在仅打印日志没有业务逻辑的方法 |
| return null 检测 | 是否存在无条件返回 null 的方法 |

**第二层：contract-validator 独立验证（R1-R5）**

| 规则 | 说明 | 级别 |
|------|------|------|
| R1 接口契约一致性 | 方法名、参数类型、返回值、异常声明与设计一致 | 警告 |
| R2 数据契约一致性 | DTO/Entity/VO 字段类型、约束、默认值与设计一致 | 警告 |
| R3 事件契约一致性 | 事件 topic、payload schema、顺序与设计一致 | 警告 |
| R4 依赖契约一致性 | 跨子任务服务依赖、API 路径与接口注册表一致 | 警告 |
| R5 逻辑覆盖率验证 | 设计文档中的逻辑单元在代码中 100% 实现 | **阻塞** |

**第三层：verify-expert 质量检查**

- 编译验证（Java: `mvn compile` / 前端: `tsc --noEmit`）
- 代码质量评分（圈复杂度、重复代码率、规范符合度）

**使用方式**：无需手动配置，每个子代理完成后自动执行。验证报告保存在 `validation-report-{taskId}.yaml` 中。

### 12.6 L5 故障自动恢复

**目标**：当子代理失败或验证不通过时，自动选择最优恢复策略，最大化交付价值。
...
| 错误类型 | 自动策略 | 说明 |
|----------|----------|------|
| 临时网络/模型错误 | 自动重试（1 次） | 附加错误信息后重新调用 |
| 编译错误 | 编译循环管理 | 自动解析错误并修复，最多 3 轮 |
| 契约不一致 | 诊断重试（1 次） | 输出诊断报告，调整参数后重试 |
| 上下文溢出 | 任务拆分 | 将大任务拆分为更小的子任务 |
| 不可恢复错误 | 人工升级 | 停止自动化，输出升级报告等待用户决策 |

**部分交付报告**

- 并行任务中部分失败时，成功任务正常交付
- 失败任务生成部分交付报告，说明已完成部分和阻塞原因
- 用户可选择修复失败后继续，或接受部分交付
...
- 解析错误类型和位置
- 生成修复方案并执行
- 最多 3 轮循环，未解决则升级到人工处理

**使用方式**：无需手动配置，系统自动根据错误类型选择策略。

### 12.7 防御体系启用条件

五层防御体系在以下场景自动启用：

- 标准模式（`/dev-flow <需求>`）：L1-L5 全部启用
- 企业级模式（`/dev-flow -subagent <需求>`）：L1-L5 全部启用，L4 验证链对每个并行子代理独立执行
- 单阶段模式（`-develop` / `-test` / `-fix`）：对应阶段涉及的防御层启用
...
五层防御体系的设计原则是"轻量前置、重验证后置"：

- **L1-L3**（生成前）：几乎无感知，自动计算和检查耗时 < 1 秒
- **L4**（生成后）：静态验证套件约 1-3 秒；contract-validator 独立验证约 5-15 秒（取决于代码量）；verify-expert 编译验证取决于项目编译时间
- **L5**（故障恢复）：仅在失败时触发，成功路径零开销

总体而言，对于成功路径，五层防御体系增加的耗时通常在 5-20 秒之间，与避免返工节省的时间相比可忽略不计。

### Q12：R5 逻辑覆盖率 100% 是如何计算的？

R5 逻辑覆盖率从 design-contract.yaml 中提取所有逻辑单元（包括：业务流程步骤、条件分支、状态转换、异常处理、数据校验规则），然后在生成的代码中逐条定位对应实现。计算公式：

```
覆盖率 = 已实现的逻辑单元数 / 设计文档中的逻辑单元总数 × 100%
```

必须达到 100% 才能通过 R5 验证。未达标的常见原因：遗漏异常处理分支、缺少状态转换逻辑、数据校验规则未实现。验证报告会列出所有未覆盖的逻辑单元及其位置。

### Q13：Checkpoint 回滚会丢失已生成的代码吗？

不会。Checkpoint 系统采用增量快照机制：

- 每个 checkpoint 保存当前已生成代码的完整状态
- 回滚时恢复到上一个稳定 checkpoint，已成功的代码完全保留
- 仅重新执行失败的部分（如某个方法填充失败，只重试该方法）
- 回滚操作由系统自动管理，无需用户手动干预

---

*本用户指南基于 dev-flow v4.0.0 编写。*