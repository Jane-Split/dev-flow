export class ApiDesigner {
    design(features, existingApis) {
        ...
        // 生成通用错误码
        errorCodes.push({ code: '400001', message: '参数错误', description: '请求参数验证失败' }, { code: '401001', message: '未授权', description: '需要登录' }, { code: '403001', message: '禁止访问', description: '无权限执行此操作' }, { code: '404001', message: '资源不存在', description: '请求的资源未找到' }, { code: '500001', message: '服务器错误', description: '服务器内部错误' });
        // 根据功能添加特定错误码
        const hasLogin = features.some(f => f.name.toLowerCase().includes('登录'));
        if (hasLogin) {
            errorCodes.push({ code: '401002', message: '账号或密码错误', description: '登录凭证不正确' }, { code: '401003', message: '账号已锁定', description: '登录失败次数过多' });
        }
        return {
            endpoints,
            errorCodes,
...
//# sourceMappingURL=api-designer.js.map