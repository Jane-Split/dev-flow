 ...
 * 运行开发流程
 */
export async function runDevFlow(options) {
 ...
 * 运行单阶段
 */
async function runSingleStage(orchestrator, options, onStageComplete) {
 ...
 * 运行全流程
 */
async function runFullFlow(orchestrator, options, onStageComplete) {
 ...
 * 交互式模式
 */
async function runInteractiveMode(orchestrator, memory, sessionId) {
    ...
    switch (action) {
            ...
            const { requirement } = await inquirer.prompt([
                {
                    ...
                    name: 'requirement',
                    message: '请输入需求描述:',
                    validate: (input) => input.trim().length > 0 || '需求描述不能为空',
                },
            ]);
        ...
        case 'stage': {
            ...
            await runSingleStage(orchestrator, { requirement: requirement || '', stage, refresh: false }, async () => true);
            break;
        }
        case 'memory': {
 ...
 * 显示记忆状态
 */
async function showMemoryStatus(memory) {
    logger.title('📊 项目记忆状态');
 ...
 * 确认阶段转换
 */
export async function confirmStageTransition(stage, current, total) {
 ...
 * 显示阶段进度
 */
export function showStageProgress(stage, progress, message) {
    const barLength = 30;
