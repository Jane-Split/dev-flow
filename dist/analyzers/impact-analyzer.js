export class ImpactAnalyzer {
    analyze(requirement, contextLinks) {
        const items = [];
        const risks = [];
        const dependencies = [];
        // 分析上下文链接
        ...
        if (lower.includes('登录') || lower.includes('login')) {
            items.push({ type: 'file', path: 'src/pages/Login/', operation: 'create', description: '登录页面' }, { type: 'api', path: '/api/auth/login', operation: 'create', description: '登录接口' }, { type: 'model', path: 'User', operation: 'modify', description: '用户模型扩展' });
        }
        if (lower.includes('用户') || lower.includes('user')) {
            items.push({ type: 'model', path: 'User', operation: 'modify', description: '用户相关模型' });
        }
        if (lower.includes('数据库') || lower.includes('database') || lower.includes('表')) {
            items.push({ type: 'database', path: 'migration', operation: 'create', description: '数据库迁移' });
