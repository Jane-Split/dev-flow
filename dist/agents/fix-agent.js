...
export class FixAgent extends BaseAgent {
    ...
    fixLogicError(content, bug) {
        ...
        fixed = fixed.replace(/(?<![=!])==(?!=)/g, (match, _offset, fullStr) => {
            ...
            const isNullCheck = rightSide === 'null' ||
                rightSide === 'undefined' ||
                rightSide === 'void 0';
            if (isNullCheck) {
                return match; // 保留原来的 ==
     ...
     * 修复依赖缺失问题
     */
    async fixDependencyError(content, bug, projectRoot) {
     ...
     * 修复 JSON 配置文件错误
     */
    fixJsonConfigError(content, bug) {
        try {
            // 尝试解析 JSON，如果失败则尝试修复
            JSON.parse(content);
    ...
    fixEnvConfigError(content, bug) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!line.includes('process.env'))
                continue;
            // 只在 bug 行附近处理
            if (bug.line !== undefined && Math.abs(i + 1 - bug.line) > 3)
                continue;
            let match;
            ...
            while ((match = envUsagePattern.exec(line)) !== null) {
                ...
                if (afterEnv.match(/^\s*(\|\||\?\?)/))
                    continue;
                // 如果在模板字符串中或字符串拼接中，跳过
                if (line.slice(0, match.index).endsWith('$') || line.slice(0, match.index).endsWith('+'))
                    continue;
                // 替换为带默认值的形式
     ...
     * 修复路径配置错误
     */
    fixPathConfigError(content, bug) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (pathMatch) {
                ...
                if (hasPathImport) {
                    ...
                    modifiedLines[i] = modifiedLines[i].replace(hardcodedPathPattern, suggestion);
                }
            }
        }
        return modifiedLines.join('\n');
     ...
     */
    addNullChecksNearLine(content, bug) {
        const lines = content.split('\n');
        ...
        const modifiedLines = lines.map((line, index) => {
            ...
            return line.replace(/(\w+)\.(\w+)/g, (match, obj, prop) => {
                ...
                if (this.isInsideTypeAnnotation(line, match))
                    return match;
     ...
     * 只在 bug 行附近添加数组边界检查
     */
    addBoundsChecksNearLine(content, bug) {
        const lines = content.split('\n');
        const targetLine = bug.line || this.findRelevantLine(lines, bug.description);
     ...
     */
    findRelevantLine(lines, description) {
        // 从描述中提取关键标识符
     ...
     * 检查匹配项是否在类型注解中
     */
    isInsideTypeAnnotation(line, match) {
    ...
    addTypeAnnotations(content) {
        ...
        // 处理 function 声明
        let fixed = content.replace(/function\s+(\w+)\s*\(([^)]*)\)/g, (match, name, params) => {
            if (!params.trim())
                return match;
        ...
        fixed = fixed.replace(/(?:const|let|var)\s+(\w+)\s*=\s*\(([^)]*)\)\s*(?:=>|{)/g, (match, varName, params) => {
            if (!params.trim())
                return match;
            const typedParams = this.annotateParams(params);
        ...
        fixed = fixed.replace(/(?<!function\s)(?<!\w)(\w+)\s*\(([^)]*)\)\s*\{/g, (match, methodName, params) => {
            // 跳过控制流关键字
