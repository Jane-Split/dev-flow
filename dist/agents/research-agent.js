...
import { StructureScanner, DependencyScanner, ComponentScanner, ApiScanner, UtilScanner, ConventionScanner, } from '../scanners/index.js';
...
export class ResearchAgent extends BaseAgent {
    ...
    generateArchitectureDoc(projectMeta, structure) {
        return `# 项目架构文档
...
## 入口文件
${structure.entryFiles.map((f) => `- ${f}`).join('\n')}
`;
    }
    formatStructure(nodes, indent = 0) {
        let result = '';
