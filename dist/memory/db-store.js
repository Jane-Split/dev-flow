...
export class DbStore {
    ...
    insertVector(entry) {
        if (!this.db)
            return;
        this.db.run(`INSERT OR REPLACE INTO vectors (id, content, embedding, type, path, name)
       VALUES (?, ?, ?, ?, ?, ?)`, [
            entry.id,
            entry.content,
    ...
    searchVectors(queryEmbedding, limit = 10) {
        if (!this.db)
            return [];
    ...
    insertFeedback(feedback) {
        if (!this.db)
            return;
        this.db.run(`INSERT INTO feedback (id, timestamp, stage, task, user_action, original_output, user_correction, learned)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
            feedback.id,
            feedback.timestamp,
            feedback.stage,
    ...
    createSession(id, metadata) {
        if (!this.db)
            return;
        this.db.run(`INSERT INTO sessions (id, created_at, status, metadata)
       VALUES (?, ?, 'active', ?)`, [id, new Date().toISOString(), metadata ? JSON.stringify(metadata) : null]);
        this.save();
    }
