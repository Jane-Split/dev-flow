...
export class TaskSplitter {
     ...
     * 创建API层任务
     */
    createApiTasks(designResult, dataTasks) {
        const tasks = [];
        ...
        for (const api of designResult.apiDesign.endpoints) {
            ...
            // 识别API依赖的数据模型
            const relatedDataTasks = dataTasks.filter(t => api.path.toLowerCase().includes(t.name.toLowerCase().replace('创建', '').replace('数据模型', '')));
            tasks.push({
                id: `task-${this.taskIdCounter++}`,
     ...
     * 创建组件层任务
     */
    createComponentTasks(designResult, apiTasks) {
        const tasks = [];
        for (const component of designResult.componentDesign.components) {
            ...
            // 识别组件依赖的API任务
            const deps = apiTasks.filter(t => component.dependencies.some((d) => d.toLowerCase().includes('api') ||
                d.toLowerCase().includes('service') ||
                t.name.toLowerCase().includes(d.toLowerCase()))).map(t => t.id);
            // 页面组件复杂度更高
            const complexity = component.type === 'page' ? 'high' : 'medium';
            tasks.push({
                id: `task-${this.taskIdCounter++}`,
     ...
     * 创建样式任务
     */
    createStyleTasks(designResult, componentTasks) {
        const tasks = [];
        // 为页面组件创建样式任务
        const pageComponents = componentTasks.filter(t => t.name.includes('页面') || t.name.includes('Page'));
     ...
     * 创建测试任务
     */
    createTestTasks(designResult, allTasks) {
        const tasks = [];
        // 获取所有实现任务的ID
     ...
     * 检查任务的上下文限制
     */
    checkContextLimits(tasks) {
        for (const task of tasks) {
            const estimatedTokens = task.context.estimatedTokens || 0;
            if (estimatedTokens > CONTEXT_LIMIT) {
                this.warnings.push(`任务 ${task.id} (${task.name}) 估计需要 ${estimatedTokens} tokens，超过限制 ${CONTEXT_LIMIT}`);
            }
        }
    }
    /**
