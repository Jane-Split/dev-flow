...
export class PythonExpert extends BaseExpert {
     ...
     * 生成 FastAPI 路由模块
     */
    generateFastApiRoute(moduleName, task) {
...
`;
     ...
     * 生成 Python 服务类
     */
    generateServiceClass(moduleName, task) {
        const entityName = this.extractEntityName(task.description);
        const entityTitle = entityName.charAt(0).toUpperCase() + entityName.slice(1).toLowerCase();
        return `"""
${task.description}
...
`;
     ...
     * 生成 Pydantic 数据模型
     */
    generateDataModel(moduleName, task) {
        const entityName = this.extractEntityName(task.description);
        const entityTitle = entityName.charAt(0).toUpperCase() + entityName.slice(1).toLowerCase();
        return `"""
${task.description}
...
`;
     ...
     * 生成 Python 模块骨架
     */
    generateModuleSkeleton(moduleName, task) {
        return `"""
${task.description}
"""
...
//# sourceMappingURL=python-expert.js.map